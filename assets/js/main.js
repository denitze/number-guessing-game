...
const getRounds = function (e) {
    if (e.target.id =="custom") {
        customInput.style.display = "block";
            customInput.addEventListener("onchange", ()=> {
    ...
    submitBtn.addEventListener("click", () => {
        ...
        if (currentRound < maxTries) {
            if (guessUser.value != randomNumber) {
                ...
                console.log("Richtig");
                result.innerHTML +=`YOU WIN! <a href="">PLAY AGAIN!</a>`
                console.log(guessUser.value);
                console.log(randomNumber);
            }
            currentRound++;
        } else if (currentRound = maxTries) {
            result.innerHTML +='Oh No! You have lost!! <a href="">TRY AGAIN?</a>'
        } 
        }
        )}
...
//eine funktion die feststellt wie viele runden schon gespielt sind bzw. wie viele runden noch gespielt werden
